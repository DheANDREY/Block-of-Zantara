...
public class NextBlock : MonoBehaviour
{
    ...
    void Start()
    {
        randomIndex = Random.Range(0, nextPrefabBlock.Length);
    ...
    public void SpawnBlockNext()
    {
        randomIndex = Random.Range(0, nextPrefabBlock.Length);
        //Debug.Log("Spawner: " + randomIndex);
        instantiatedPrefab = Instantiate(nextPrefabBlock[randomIndex], new Vector3(nextBlockSlot.position.x, (float)(nextBlockSlot.position.y - 0.35), nextBlockSlot.position.z), Quaternion.identity) as GameObject;
    ...
    private void changeBlock()
    {
        ...
        if (SpawnerBlock.instance.isBlockSpawned)
        {
            ...
            currentIndex++;
            if (currentIndex >= nextPrefabBlock.Length)
            {
                currentIndex = -1;
            }
            SpawnBlockNext();
